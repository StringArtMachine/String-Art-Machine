...
void setup() {
  ...
  server.on("/upload", HTTP_POST, [](AsyncWebServerRequest *request) {
    ...
    AsyncWebServerResponse *response =
        request->beginResponse(200, "text/plain", "CORS headers included");
    response->addHeader("Access-Control-Allow-Origin", "*");
    response->addHeader("Access-Control-Allow-Methods", "POST, OPTIONS");
    response->addHeader("Access-Control-Allow-Headers", "Content-Type");
  ...
  server.on("/start-or-resume", HTTP_POST, [](AsyncWebServerRequest *request) {
    ...
    stateTicker.attach(0.5, updateMachineState); 
  ...
  server.on("/restart", HTTP_POST, [](AsyncWebServerRequest *request) {
    ...
    prefs.clear();  // This will clear all preferences saved under the current
                    // namespace
  ...
  server.on("/increment-move", HTTP_POST, [](AsyncWebServerRequest *request) {
    if (request->hasParam("direction")) {
      ...
      if (direction == "cw") {
        ...
        stepper.move(
            25);  // Increment by 640 microsteps (1/10th of a revolution)
      } else if (direction == "ccw") {
      ...
      // Send response
      request->send(200, "application/json",
                    "{\"message\":\"Motor increment move command sent\"}");
    } else {
      request->send(400, "application/json",
                    "{\"error\":\"Direction not provided\"}");
    }
  ...
  server.on("/stop-manual-calibration", HTTP_POST,
  ...
  server.on("/start-drilling", HTTP_POST, [](AsyncWebServerRequest *request) {
    if (isDrilling) {
      request->send(400, "application/json",
                    "{\"error\": \"Drilling is already in progress.\"}");
      return;
    }
    ...
    isDrilling = true;
    drillProgress = 0;
    // Start the timer to update progress every 100ms (or any desired interval)
    drillingTimer.attach(0.1, updateDrillingProgress);  // Update every 100ms

    // Respond immediately to indicate the drill has started
    request->send(200, "application/json",
                  "{\"status\": \"Drilling started\"}");
  });
  
  // Handle stopping the drilling (for testing)
  server.on("/stop-drilling", HTTP_POST, [](AsyncWebServerRequest *request){
    isDrilling = false;
    drillProgress = 0;
  
    // Stop the timer since drilling has stopped
    drillingTimer.detach();
  
    request->send(200, "application/json", "{\"status\": \"Drilling stopped\"}");
  });
}

void loop() { stepper.run(); }

// Function to simulate the drilling process
void startDrillingProcess() {
  if (isDrilling) {
    // Simulate drilling progress by incrementing
    if (drillProgress < 100) {
      drillProgress++;
    }
  }
}
// Function to periodically call the startDrillingProcess
void updateDrillingProgress() {
  startDrillingProcess();  // Update the drilling progress
}

void updateMachineState() {
    // Simulate progress and machine states
    if (machineRunning) {
        // Increment the progress as the machine runs
        if (progress < 100) {
            progress++;
        }
        
        // Simulate drilling progress
        if (isDrilling && drillProgress < 100) {
            drillProgress++;
            drillPosition++; // Update the drill position
        }

        // Simulate movement (this can be used to update the stepper position)
        stepperPosition++;

        // Simulate line completion (for example, one line per 10% of progress)
        if (progress % (100 / totalLines) == 0 && currentLine < totalLines) {
            currentLine++;
            linesCompleted++;
            currentMove = "Moving to line " + String(currentLine);
        }
    }
}
